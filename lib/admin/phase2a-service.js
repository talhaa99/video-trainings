            ...
            (
              SELECT json_agg(
                json_build_object(
                  ...
                  'moduleIndex', seq.module_index,
                  'totalQuestions', seq.total_questions,
                  'source', seq.source_label
                )
              ...
              FROM (
                SELECT
                  ...
                  (l.event_payload->>'moduleIndex')::INT AS module_index,
                  (l.event_payload->>'totalQuestions')::INT AS total_questions,
                  l.event_payload->>'source' AS source_label
...
export async function trackAssignmentEventByToken({ token, eventType, payload = {} }) {
  ...
  if (eventType === 'module_completed') {
    ...
    const moduleIndexRaw = Number(payload?.moduleIndex)
    if (!Number.isFinite(moduleIndexRaw) || moduleIndexRaw < 0 || moduleIndexRaw > 1) {
      throw new Error('Invalid module index.')
    }
    const modulesTotalRaw = Number(payload?.modulesTotal)
    await addAssignmentEvent({
      assignmentId: assignment.id,
      eventType: 'module_completed',
      payload: {
        moduleIndex: moduleIndexRaw,
        modulesTotal: Number.isFinite(modulesTotalRaw) ? modulesTotalRaw : null,
        videoId: payload?.videoId != null ? String(payload.videoId) : null,
      },
    })
    return { ok: true }
  }
  ...
  if (eventType === 'quiz_submitted') {
    ...
    if (assignment.training_type === 'general_training') {
      if (payload?.source !== 'module_quiz') {
        throw new Error('General training records module quizzes via the training app only.')
      }
      if (assignment.completed_at) {
        return { ok: true }
      }
      ...
      await addAssignmentEvent({
        ...
        payload: {
          ...
          quizPassed,
          totalQuestions: Number.isFinite(totalQuestionsRaw) ? totalQuestionsRaw : null,
        },
      })
      return { ok: true }
    }
